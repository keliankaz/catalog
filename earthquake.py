...
class EarthquakeCatalog(Catalog):
    def __init__(
        ...
        """Generates an EarthquakeCatalog either from a pandas csv, a new catalog query using the obspy or from an existing Catalog.
        ...
        earthquakes = EarthquakeCatalog(
            filename="global_earthquakes_7.csv",
            kwargs=earquake_metadata,
        )
        
        Inputs:
            catalog: Input catalog that is passed to the `Catalog` class
            filename: File name that should be specified if querying a catalog. 
                The default behavior is to load the exsisting csv file with the 
                corresponding name if it exists unless `reload` is `True`.
            use_other_catalog: set to True if querying a new obspy catalog around an existing Catalog.
                The new `EarthquakeCatalog` will comprise events within `other_catalog_buffer` (a multiplicative
                factor applied to the latitude and longitude range) around the `other_catalog`.
            kwargs: key word arguments passed to `EarthquakeCatalog.get_and_save_catalog`.
            other_catalog: Catalog to used as reference to querry new catalog. 
            other_catalog_buffer: buffer factor applied to the latitude and longitude range of `other_catalog`.
            reload: Set to True or False to specify whether or not to make a new query if there alrady exists
                as filename with the corresponding filename.
        
        Output:
            A new instance of an EarthquakeCatalog        
